...
from src.utils.utilities import download_arxiv_pdf, parse_arxiv_pdf, parse_script_plan, get_paper_head, initialize_discussion_chain
...
def process_section(state: PodcastState) -> Dict[str, Any]:
    ...
    section_script = discuss_chain.invoke({
        "section_plan": current_section,
        "previous_dialogue": state["script"]
    })
    remaining_sections = state["section_plan"][1:]
    return {
        "script": section_script,
        "section_plan": remaining_sections
    }
